...
int parser(char * toParse) {
   ...
   PLL * transf = malloc(sizeof(PLL));
   PLL * final_corr = transf;
	...
	while (ptr != NULL && !is_empty(ptr))
	{
      ...
      if(ptr2 != NULL){
         ...
         //printf("#3 %s\n", ptr2);
         TreeLL * init_child = malloc(sizeof(TreeLL));
         Node * node_init = malloc(sizeof(Node));
         init_child->n = node_init;
         TreeLL * headChild = init_child;
   
         while (ptr2 != NULL)
         {
            printf("#9999\n");
            memcpy(init_child->n->title, ptr2, sizeof(init_child->n->title));
            TreeLL * tmp_child = malloc(sizeof(TreeLL));
            
            printf("#4321\n");
            ptr2 = strtok_r(NULL, delim5, &saveptr2);
            printf("#1234\n");
            if (ptr2 != NULL){
               Node * node_init2 = malloc(sizeof(Node));
               tmp_child->n = node_init2;
         ...
         PLL * tmp_corr = malloc(sizeof(PLL));
         memcpy(transf->name, tmp->title, sizeof(transf->name));
         transf->next = tmp_corr;
         transf->t = initTree;
         transf = tmp_corr;
   ...
   TreeLL * finalTree = malloc(sizeof(TreeLL));

   TreeLL * realHeadTree = headTree;

   TreeLL * superrealHeadTree = finalTree;
   ...
   /*while(headTree->next != NULL){
      ...
      while(headTree->children != NULL){
         printf("#++++++++ %s\n", headTree->children->n->title);

         PLL * runner_PLL = final_corr;
         while(runner_PLL != NULL){
            //printf("PLL IS : %s\n",runner_PLL->name);
            if ( strcmp(runner_PLL->name, headTree->children->n->title) == 0 ){
               printf("FOUND IT : %s ; %s\n", runner_PLL->name, headTree->children->n->title);
               superrealHeadTree->children = runner_PLL->t;
            }

            runner_PLL = runner_PLL->next;
         }   
         superrealHeadTree->children->next;
         headTree->children = headTree->children->next;
      }
      superrealHeadTree = superrealHeadTree->next;
      headTree = headTree->next;
   }*/
   /*
      if(checkIn(transf, ptr2)){
      print("YOOOdddddddddddddddddddOO\n");
   }
   */
